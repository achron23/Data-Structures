...
List state_objects(State state, float x_from, float x_to) {
	...
	ListNode node=LIST_BOF;    //iconic node before first
	for (int i=0;i<vector_size(state->objects);i++){
		Object a=vector_get_at(state->objects,i);     //a is a cast from Pointer to Object   
		if(a->rect.x >= x_from && a->rect.x <= x_to){
			list_insert_next(list,node,vector_get_at(state->objects,i));  //inserts to the list the objects needed
			node = list_last(list);
